...
larger for smaller objects such as stars and distant galaxies, the images of
which are more affected by the PSF.
...
and the same shape as the original image.  We apply \mcal\ shearing and
convolution operations.  We then rotate this field by 90 degrees and add it to
the image of the object before performing a measurement, which cancels the
correlated noise bias in the ensemble measurement \citep{SheldonHuff2017}.
...
correction scheme.  We generate noise images as described above, and pass them
through coaddition, thus incorporating correlated noise due to interpolation.
We then pass these images through the shearing and convolution operations and
make the empirical correlated noise correction.
