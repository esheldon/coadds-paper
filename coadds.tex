...
\subsection{Monte-carlo Tests For Template Fluxes} \label{sec:toymc}
...
The primary variable of interest is the resolution $R$.  We varied the
size of the galaxy from zero, or star-like, to more than twice the PSF size,
such that $R = 0.83$.

The results are shown in figure \ref{fig:mcresults}.  The points are the
measured values, which are predicted very well by the exact variance
estimators, shown as the solid line.  Also shown are the predicted values from
the toy model in equation \ref{eq:vargal}.  As expected, the toy model slightly
over-predicts the increase in variance, but it is evidently useful for
understanding how the effect scales with resolution.
...
\begin{figure}
    ...
    uncertainty due to coaddition, for the Gaussian galaxies
    and stars.  The points are
    the measured values, and the solid line is the exact prediction.
    The dashed line shows the simple toy model presented
    in equation \ref{eq:vargal}.}
...
This needs to be rewritten, since it actually is the affect of a noisy template.
...
\begin{align}
    {\rm var}\hat{A} &= \Bigl< (A - \hat{A})^2 \Bigr>
\end{align}
Which we can write in terms of an outer product
\begin{align}
    {\rm var}\hat{A} &= \frac{1}{N} (A - \hat{A}) (A - \hat{A})^T \\
        &= \frac{1}{N} \left(A \frac{\boldsymbol{m}_c C_c^{-1} \Delta \boldsymbol{m}}{\boldsymbol{m}_c C_c^{-1}\boldsymbol{m}_c} 
              ...
              + \frac{\boldsymbol{m}_c C_c^{-1} \boldsymbol{\epsilon} }{\boldsymbol{m}_c C_c^{-1}\boldsymbol{m}_c}\right)^T
\end{align}
...
\begin{align} 
    {\rm var}\hat{A} &= {\rm var}\hat{A_c} \Bigl[ 1 + 
        \frac{1}{N}\frac{A^2}{\boldsymbol{m}_c^T C_c^{-1}\boldsymbol{m}_c}  \left(\boldsymbol{m}_c^T C_c^{-1} \Delta \boldsymbol{m} \right)  \left(\boldsymbol{m}_c^T C_c^{-1} \Delta \boldsymbol{m} \right)^T \Bigr] \\
        &= {\rm var}\hat{A_c} \Bigl[ 1 + \frac{1}{N}\frac{A^2}{\boldsymbol{m}_c^T C_c^{-1}\boldsymbol{m}_c} \boldsymbol{m}_c^T C_c^{-1} \Delta \boldsymbol{m} \Delta \boldsymbol{m}^T  C_c^{-1} \boldsymbol{m}_c  \Bigr],
\end{align}
...
\begin{align}
    {\rm var}\hat{A} &= {\rm var}\hat{A_c} \Bigl[ 1 + 
        \frac{1}{N}\frac{A^2}{\eta^2} \frac{\Delta \sigma^2}{\boldsymbol{m}_c^T \boldsymbol{m}_c} \boldsymbol{m}_c^T  \frac{\partial \boldsymbol{m}}{\partial \sigma} \frac{\partial \boldsymbol{m}}{\partial \sigma}^T  \boldsymbol{m}_c  \Bigr]
\end{align}
...
\begin{align}
    {\rm var}\hat{A} &= {\rm var}\hat{A_c} \Bigl[ 1 + 
        \frac{16}{N}\frac{A^2}{4 \pi \sigma^2 \eta^2} \Bigl( \frac{\Delta \sigma}{\sigma} \Bigr)^2 \boldsymbol{m}_c^T  \boldsymbol{F} \boldsymbol{F}^T \boldsymbol{m} \Bigr]
\end{align}
...
Additional, the quantity $\boldsymbol{m}_c^T  \boldsymbol{F} \boldsymbol{F}^T \boldsymbol{m}$
...
When fitting, the coadd template was taken to be the best fit gaussian, rather
than the exact coadded template from the individual gaussian images.  As we
will see below, this may introduce some small additional variance.
