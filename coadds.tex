...
We have shown that we can reliably infer weak lensing shear on coadded images 
using two state-of-the art shear techniques, BFD and \mcal.  For these tests
we used a suite of simulations.  We see minimal loss of
loss of information using non-optimal weighted mean coadds for relatively
small PSF size variations.  The fact that we have shown 
this with two different methods gives support that is could be generally 
applicable to other shear methods as well.

We identify the key issue for recovering accurate shear is careful handling of
the correlated noise that arised due to coaddition.  We have explictily assumed
in this study that the coadded noise field is roughly constant over the size of
an object.  The scale of noise variation will depend on the distortion,
...
approach may not be optimal.  For example, such simple coadds will likely
degrade the ability to perform precise star-galaxy separation, as the width of
the coadd PSF is necessarily larger than that of the best input images.  In
this case there is  clear benfit to constructing optimal coadds.  Exploring this
in more detail is beyond the scope of this work.
...
relatively small to avoid rejecting too many exposures.  We examined the typical size
of blended regions from the HSC Ultra-Deep field and found that the number of
