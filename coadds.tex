...
\cite{Jee2016} and \cite{Mandelbaum2018} is to create a valid coadded PSF by resampling 
and interpolating the single epoch PSFs in the same way as the images 
\cite{DLS, HSC}.
\rachel{May be more appropriate to cite the HSC pipeline paper rather than the shear catalog paper
...
\end{align}
where $\sigma_p$ is the PSF size,  \rachel{mean PSF size?} $\Delta \sigma_p$ is the RMS scatter in PSF
...
factor of 2--3 for star-like objects.  However, most current and future
surveys have significantly less PSF variation than this, on the order of 0.1.
\rachel{For what it's worth, if I take the Cerro Pachon seeing distribution from the LSST SRD, I
  find mean seeing of 0.67 arcsec and standard deviation of 0.3, so $\Delta \sigma_p/\sigma_p \sim
  0.45$.  Accounting for variation in zenith angle would presumably broaden this more.  I'm not sure
this is so atypical, and it's closer geometrically to the original example you gave (1) than the one
you say is typical (0.1).}
...
\begin{itemize}
\item We generate single galaxy images on $48 \times 48$ postage stamps with no 
neighbors.
...
distribution with mean 0.95 and $\sigma=0.1$.  Values are truncated between 
0.8 and 1.15.
...
\item The intrinsic ellipticity is drawn from the distribution $P(e) \propto 
  (-e^2)^2 \exp{(-e^2/2\sigma_e^2)}$ with $\sigma_e=0.02$, where $e$ is defined
  as a shear.  The small width here is set to reduce the shape noise.
\rachel{This is a slightly odd way to write the $P(e)$ and I am wondering if there is a typo?  (I'm
  not sure why to write $(-e^2)^2$ instead of just $e^4$\dots)}
...
\rachel{I am not following this last sentence, specifically the ``we doubled measured moment noise''
part - can you explain / rephrase?}
...
\rachel{Were those cuts made on the coadd and used consistently for the coadd vs. multi-epoch
  measurements?} For BFD we made cuts XXX.
\rachel{Flagging this missing info re: BFD cuts, in case this fell through the cracks.} We
...
This is likely because in BFD we were making the same error in the deep reference
set as in the shallow images, so this problem was being calibrated out.
\rachel{Interesting.  This is kind of a side point for this paper but I'm curious to think through
  what this mean in terms of the ability to self-calibrate some systematics using BFD (which ones
  can be self-calibrated in this way because they affect the deep images).}
...
\caption{
Two different LSST dither patterns are shown taken from \cite{LSSTSims}.  
...
the reduced 5$\sigma$ point source depth for each patch.  Fig.~\ref{fig:cell} 
...
patches to avoid losing many visits.  \rachel{This figure shows the 5-sigma point source depth in
  which band?  I'm trying to figure out what numbers to compare it to.}
...
to the wide field LSST depth.
\rachel{Is this statement specific to the Ultra-Deep data in PDR1?  My expectation is that when the
  HSC survey is complete, HSC UD will be significantly deeper than LSST Wide, while HSC Deep will be
comparable to LSST Wide.  But that might not be true in the first data release?}
...
of 5$\sigma$ depths for each patch. \rachel{Legend should have arcmin symbol rather than single
  quote mark. Also, I suggest switching to a color blind-friendly color
  palette, like seaborn ``colorblind'' (https://seaborn.pydata.org/tutorial/color\_palettes.html) or
https://jiffyclub.github.io/palettable/tableau/\#colorblind\_10}
}
\label{fig:cell}
\end{figure*}
...
effective survey depth.  It is not clear, however, that a multi-epoch fitting
...
\rachel{Is there some comment we can make on potential computational obstacles / costs associated
  with a ``per-object coadd'' or ``coadds in cells'' approach, compared to the approaches that have
  been discussed more extensively? (i.e., coadds that are not per object / not in cells;
  multi-fitting)  I think this would be a useful addition to the final section of the paper.}
...
\rachel{Is there a punch line so we can end on a high note?  Here's my attempt at one, for what it's
worth: ``The weak lensing literature includes studies that take a wide variety of approaches to shear
...
well-justified in the use of coadds for shear inference.''  Feel free to use/modify/discard as you
wish!}

\section*{Acknowledgments}

\rachel{Funding and other acknowledgments here?}
